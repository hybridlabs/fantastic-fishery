...
class FFConfigHandler(val file: File, val defaultConfig: FFConfig = FFConfig()) {
    val backupFile = file.parentFile.resolve("${file.name}-backup")

    var config: FFConfig = defaultConfig

    /**
     * Reads the config from the config file.
     * @return a data result of the config
     */
    fun read(): DataResult<Pair<FFConfig, JsonElement>> {
        val json = file.reader().use(JsonParser::parseReader)
        return FFConfig.CODEC.decode(JsonOps.INSTANCE, json)
    }
    ...
    fun save(): Boolean {
        val dataResult = FFConfig.CODEC.encodeStart(JsonOps.INSTANCE, config)
        val result = dataResult.result()
